...
void Device::AddEngine(EnginePtr_t engine)
{
    ...
    // FIXME: only supporting batchsize == 1, for now
    CHECK_EQ(m_BatchSize, 1);
}
...
void Server::Setup(ServerSettings_3DUNet& settings, ServerParams& params)
{
            ...
            for (auto& batches : deviceTypes)
            {
                     ...
                     deviceInstance++)
                {
                    ...
                    auto device = std::make_shared<lwis::Device>(deviceNum, numCopyStreams, numInferStreams,
                        ...
                        m_ServerSettings.UseSameContext);
    ...
    for (auto& device : m_Devices)
    {
        ...
        m_kits19_buffer_manager[deviceNum] = std::make_shared<KiTS19BufferManager>(m_ServerSettings.SW_dhw,
            ...
            directHostAccess, deviceNum, numaIdx, numNUMADomains);
    }
    ...
    std::deque<mlperf::QuerySample>::iterator end, cudaStream_t copyStream)
{
    ...
    std::vector<std::vector<void*>> sw_buffers;
    ...
    // loop -- sliding window
    // NOTE: support batchsize == 1 inside sliding window for now
    //       reason is better efficiency with multiple inference streams than batchsize > 1
    // NOTE: stride is guaranteed to be smaller than SW_dhw, i.e. sliding window d/h/w
    bool first_sw = true;
    const int stride = m_ServerSettings.SW_dhw * m_ServerSettings.SW_overlap_pct / 100;
    for (int d = 0; d < sample_dims.d - stride; d += stride)
    {
        int dd = d == 0 ? 0 : (d == sample_dims.d - m_ServerSettings.SW_dhw ? 2 : 1);
        for (int h = 0; h < sample_dims.h - stride; h += stride)
        {
            int hh = h == 0 ? 0 : (h == sample_dims.h - m_ServerSettings.SW_dhw ? 2 : 1);
            for (int w = 0; w < sample_dims.w - stride; w += stride)
            {
                int ww = w == 0 ? 0 : (w == sample_dims.w - m_ServerSettings.SW_dhw ? 2 : 1);

                int patch_idx = dd * 9 + hh * 3 + ww;
                ...
                // for input & output hence size==2
                sw_buffers[device->m_InferStreamNum].resize(2);
                ...
                // UNet3DKiTS19SliceKernelI8CDHW32_wrapper(sample_in_buf,
                UNet3DKiTS19SliceKernelI8Linear_wrapper(
                    sample_in_buf, sw_buffers[device->m_InferStreamNum][0], &u3p, inferStream, deviceId);
                TIMER_END(UNET3DSW_sliceKernel);

                TIMER_START(enqueueShim);
                enqueueShim(context, enqueueBatchSize, &sw_buffers[device->m_InferStreamNum][0], inferStream, nullptr);
                TIMER_END(enqueueShim);
                // NOTE: patchKernel's Accumulation could be in race condition if multi-inferstream
                // is used
                //       therefore need a serializing point
                TIMER_START(UNET3DSW_patchKernel);
                UNet3DKiTS19PatchKernel_wrapper(sw_buffers[device->m_InferStreamNum][1],
                    KiTS19bufMgr->getGaussianPatchBuffer(patch_idx), sample_out_buf, &u3p, inferStream, deviceId);
