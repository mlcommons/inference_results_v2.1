...
public:
  AppenedSampleLoader(std::string name, std::string map_file_path /*the result value map file path*/,
                      ...
                      const std::vector<bool>& start_from_device, std::size_t device_idx, std::size_t padding,
                      std::size_t sample_alignment)
    : name_(name),
      perf_sample_count_(perf_sample_count),
      perf_sample_padding_(padding),
      sample_data_alignment_(sample_alignment),
    ...
    for (std::size_t input_idx = 0; input_idx < num_inputs_; input_idx++) {
      ...
      for (std::size_t idx = 0; idx < dim_size; ++idx) {
        getMallocFun(start_from_device_[input_idx])(&sample_memory_[input_idx][idx],
                                                    (perf_sample_count_)*dim_sizes_[input_idx][idx]);
      }
    }
  }
  ...
  void LoadSamplesToRam(const std::vector<mlperf::QuerySampleIndex>& samples) override {
    ...
    for (std::size_t sampleIndex = 0; sampleIndex < samples.size(); sampleIndex++) {
      ...
      for (std::size_t input_idx = 0; input_idx < num_inputs_; input_idx++) {
        ...
        if (trim_last_zeros_) {
          origin_data_len = data_len = getDataLenWithoutLastZeros<int32_t>(data_ptr, data.size());
          sample_origin_lens_[input_idx][sampleId] = origin_data_len;

          auto ratio = sizeof(int32_t);
          origin_data_len *= ratio;
          data_len *= ratio;
          if (sample_data_alignment_ != 0) {
            data_len = ALIGN(data_len, sample_data_alignment_);
          }
          sample_lens_[input_idx][sampleId] = data_len / ratio;
        } else {
          origin_data_len = data_len = data.size();
          sample_origin_lens_[input_idx][sampleId] = origin_data_len;
          if (sample_data_alignment_ != 0) {
            data_len = ALIGN(data_len, sample_data_alignment_);
          }
          sample_lens_[input_idx][sampleId] = data_len;
        }

        for (std::size_t dim_idx = 0; dim_idx < dim_cnt; ++dim_idx) {
          // The size of data to copy use the min of dims size and data size.
          std::size_t size_to_copy = std::min(origin_data_len, data.size());
          ...
          auto sampleAddress = static_cast<int8_t*>(sample_memory_[input_idx][dim_idx]) + current_idx * dims[dim_idx];
...
private:
  ...
  std::size_t sample_data_alignment_{0};
