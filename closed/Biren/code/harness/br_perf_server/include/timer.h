...
PackedSequence<T> pack_one_instance(const SequenceList<T>& multi_sequence, int max_sequence_length,
                                    ...
                                    std::unordered_map<mlperf::QuerySample*, std::size_t>& samples_to_len) {
  PackedSequence<T> packed;
  auto offset = 0;
  packed.samples.reserve(multi_sequence.size());
  packed.masks.reserve(max_sequences_per_pack * 2);
  for (const auto& seq : multi_sequence) {
    packed.samples.push_back(seq.sample);
    packed.masks.push_back(offset);
    packed.masks.push_back(offset + samples_to_len[seq.sample]);
    offset += seq.length;
  }
  for (size_t idx = packed.masks.size(); idx < max_sequences_per_pack * 2; ++idx) {
    packed.masks.push_back(0);
  }
  return packed;
}

template<typename T>
PackedSequenceList<T> pack(LenToSampleMap& examples_by_length, PackList& packList,
                           std::unordered_map<mlperf::QuerySample*, std::size_t>& samples_to_len,
                           int max_sequence_length, int max_sequences_per_pack) {
  ...
  for (const auto& pl : packList) {
    ...
    for (auto i = 0; i < pl.count; ++i) {
      ...
      auto* p = new PackedSequence<T>(
        pack_one_instance(multi_sequence, max_sequence_length, max_sequences_per_pack, samples_to_len));
