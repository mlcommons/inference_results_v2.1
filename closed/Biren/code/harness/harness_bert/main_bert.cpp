...
auto BertDataProcess = [](brPerfServer::BindingHandle* input_binding, const std::vector<std::size_t>& real_input_size,
  ...
  for (std::size_t input_idx = 0; input_idx < input_cnt; ++input_idx) {
    ...
    besuMemset(buff_ptr, 0, alignment * compute_unit_num);
    ...
    for (std::size_t unit_idx = 0; unit_idx < compute_unit_num && unit_idx < pack_list.size(); ++unit_idx) {
      if (LIKELY(input_idx != BertPosMaskIdx)) {
        ...
        last_offset = 0;
        data_idx = 0;
        for (auto data_idx = 0; data_idx < one_pack.size(); data_idx += 2) {
          if (data_idx != 0 && one_pack[data_idx] == 0) {
            continue;
          }
          auto data_start = one_pack[data_idx];
          auto data_len = one_pack[data_idx + 1] - data_start;
          // The even one in packet is the length of data_size [0, data_size - 1).
          besuMemcpy(buff_ptr + data_start * sizeof(BertInputDataType), input_datas.dim_datas[sample_idx++][0],
                     data_len * sizeof(BertInputDataType));
        }
        buff_ptr += alignment;
      } else {
    ...
    [&responses, compute_units, batch, infer_result_data, output_data_size](mlperf::QuerySampleResponse* response) {
      ...
      if (batch->packlist.empty()) {
        ...
        for (auto pack : batch->packlist) {
          ...
          auto data_start = 0;
          auto data_len = 0;
          for (auto data_idx = 0; data_idx < pack.size(); data_idx += 2) {
            if (data_idx != 0 && pack[data_idx] == 0) {
              continue;
            }
            if (response->id == (*responses)[sample_idx].id) {
              data_start = pack[data_idx];
              data_len = (pack[data_idx + 1] - data_start) * sizeof(BertOutputDataType) * 2;
              response->data = (uintptr_t)(pack_result + data_start * sizeof(BertOutputDataType) * 2);
  ...
  for (int32_t device_idx : devices_list) {
    auto createLoader = [&]() {
      ...
      auto loader = std::make_shared<samplesLoader::AppenedSampleLoader>(
        ...
        serverSettings.dim_sizes, trim_zero_endings, start_from_device, device_idx, padding, 0);
