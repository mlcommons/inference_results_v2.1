...
const char* images_bin_path = "/work/build/preprocessed_data/imagenet";
const std::string output_bin_path =
  "/work/build/preprocessed_data/imagenet_int8_new_block0";
...
void thread_worker(const std::string& filepath) {
  ...
  suinfer::sugraph::Shape o_shape(1, 256, 256, 3); // NCHW
  ...
  // left, top --> 3, right, down --> 29
  // left , up, right, down
  suinfer::sugraph::Shape padding(3, 3, 29, 29);
  ...
  pad<3, 224, 224, 3, 3, 29, 29>(chw_data.data(), padding_data.data());

  size_t o_shape_length = o_shape.length();
  std::vector<uint8_t> hwc_data(o_shape_length, 0);
  CHW2HWC<256, 256, 3>(padding_data.data(), hwc_data.data());
