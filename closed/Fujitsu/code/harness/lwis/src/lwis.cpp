    ...
    for (auto& e : m_Engines)
    {
        ...
        while (batchSize <= maxBatchSize)
        {
            for (auto& streamState : m_StreamState)
            {
                ...
                auto buffers = directHostAccess ? &(bufferManager->getHostBindings()[0])
                                                : &(bufferManager->getDeviceBindings()[0]);
    ...
    std::deque<mlperf::QuerySample>::iterator end, cudaStream_t copyStream)
{
    ...
    if (m_ServerSettings.EnableResponse)
    {
        ...
        if (m_ServerSettings.EndOnDevice)
        {
            auto baseDevice = reinterpret_cast<uintptr_t>(bufferManager->getDeviceBuffer(bindingIdx));
            auto baseHost = reinterpret_cast<uintptr_t>(bufferManager->getHostBuffer(bindingIdx));
            ...
            if (!contiguous)
            {
                ...
                for (auto it = begin; it != end; ++it)
                {
                    if (m_ServerSettings.EnableStartFromDeviceMem)
                    {
                        ...
                        CHECK_EQ(cudaMemcpyAsync(static_cast<int8_t*>(bufferManager->getDeviceBuffer(i))
                                         + offset++ * sampleLibrary->GetSampleSize(i),
                                     sampleLibrary->GetSampleAddress(it->index, i, deviceId),
                                     sampleLibrary->GetSampleSize(i), cudaMemcpyDeviceToDevice, stream),
                            cudaSuccess);
                    ...
                    else if (directHostAccess)
                    {
                        ...
                        memcpy(static_cast<int8_t*>(bufferManager->getHostBuffer(i))
                                + offset++ * sampleLibrary->GetSampleSize(i),
                            sampleLibrary->GetSampleAddress(it->index, i), sampleLibrary->GetSampleSize(i));
                    ...
                    else
                    {
                        ...
                        bufferManager->copyInputToDeviceAsync(i, stream, sampleLibrary->GetSampleAddress(it->index, i),
                            sampleLibrary->GetSampleSize(i), offset++);
                        device->m_Stats.m_PerSampleCudaMemcpyCalls++;
                    }
                }
            }
            else
            {
                if (m_ServerSettings.EnableStartFromDeviceMem)
                {
                    if (!m_ServerSettings.EnableCudaGraphs)
                    {
                        ...
                        {
                            buffers[i + profile_idx * num_bindinges_per_profile]
                    ...
                    else
                    {
                        ...
                        CHECK_EQ(cudaMemcpyAsync(static_cast<int8_t*>(bufferManager->getDeviceBuffer(i)),
                                     sampleLibrary->GetSampleAddress(begin->index, i, deviceId),
                                     sampleLibrary->GetSampleSize(i) * batchSize, cudaMemcpyDeviceToDevice, stream),
                            cudaSuccess);
                ...
                else if (directHostAccess)
                {
                    if (!m_ServerSettings.EnableCudaGraphs)
                    {
                        ...
                        {
                            buffers[i + profile_idx * num_bindinges_per_profile]
                                = sampleLibrary->GetSampleAddress(begin->index, i);
                        }
                    }
                    else
                    {
                        // with cuda graphs, we have to do H2H copies because we cannot change bindings for graphs
                        memcpy(bufferManager->getHostBuffer(i), sampleLibrary->GetSampleAddress(begin->index, i),
                            sampleLibrary->GetSampleSize(i) * batchSize);
                    }
                }
                else
                {
                    // copy direct to device buffer with single DMA
                    bufferManager->copyInputToDeviceAsync(i, stream, sampleLibrary->GetSampleAddress(begin->index, i),
                        sampleLibrary->GetSampleSize(i) * batchSize);
                    device->m_Stats.m_BatchedCudaMemcpyCalls++;
                }
            }
...
void Server::ProcessSamples()
{
    ...
    while (true)
    {
        ...
        do
        {
            ...
            m_WorkQueue.acquire(samples, m_ServerSettings.Timeout, device->m_BatchSize,
