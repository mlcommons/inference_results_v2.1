...
public:
    ...
    KiTS19BufferManager(const int SW_dhw = 128, const int numInferStream = 1, const size_t maxSampleSize = 64225280,
        ...
        const int NUMAidx = -1, const int numNUMA = 1)
        ...
        , mNumNUMA(numNUMA)
    {
        ...
        // Each optimization profile owns numBindings bindings.
        // Create host and device buffers
        // SW: input is 1ch INT8, output is 3ch FP16
        size_t sw_vol = SW_dhw * SW_dhw * SW_dhw;
        size_t sw_input_alloc = sw_vol * /*int8*/ 1 * /*1ch*/ 1;
        size_t sw_output_alloc = sw_vol * /*fp16*/ sizeof(__half) * /*3ch*/ 3;
        ...
        size_t sample_input_alloc = maxSampleSize * /* INT8 */ 1 * /* 1ch */ 1;
        size_t sample_output_alloc = maxSampleSize * /* FP16 */ 2 * /* 3ch */ 3;
        size_t sample_resp_alloc = maxSampleSize * /* INT8 */ 1 * /* 1ch */ 1;
    ...
    // NOTE: FIXME: sample batch size == 1 && SW batch size == 1 for now
        ...
        const bool async = false)
    {
        ...
        mSampleInputSize = sample_vol * /* INT8 */ 1 * /* 1ch */ 1;
        mSampleOutputSize = sample_vol * /* FP16 */ 2 * /* 3ch */ 3;
        mSampleResponseSize = sample_vol * /* INT8 */ 1 * /* 1ch */ 1;
