...
- The docker container does not copy the files, and instead **mounts** the working directory (closed/HPE) under /work in the container. This means you can edit files outside the container, and the changes will be reflected inside as well.
...
### Running the actual benchmark on Champollion
Our repository has convenience methods for running the benchmark on Champollion. For example, a batch job can be run with:

```
sbatch -N 1 ./run.docker.sh /work/run_all.sh
```

### Running the actual benchmark per NVIDIA directions
The NVIDIA repo has one main command to run any of our benchmarks:
