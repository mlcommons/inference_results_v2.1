    ...
    def get_first_match(cls, x) -> Matchable:
        ...
        for elem in cls:
            if elem.value.matches(x):
                return elem
            elif 'A100-SXM-80GB MIG-1g.10gb' in str(elem.value) and "7763" in str(elem.value.pretty_string()):
            # print(f"Does not match:\n\nKNOWN:{x}\n\nMINE:{elem.value}")
                if x.host_cpu_conf == elem.value.host_cpu_conf:
                    #print("CPU matches")
                    if x.accelerator_conf == elem.value.accelerator_conf:
                        #print("Accelerator conf matches")
                        if str(x.host_mem_conf.pretty_string()).split(' ') ==str(elem.value.host_mem_conf.pretty_string()).split(' ')[1:]:
                            
                            #print("Host MEM matches")
                            return elem

                        else:
                            print(f"Host mem does not match: {x.host_mem_conf.pretty_string()} vs {elem.value.host_mem_conf.pretty_string()}")
                            print(str(x.host_mem_conf.pretty_string()).split(' '))
                            print(str(elem.value.host_mem_conf.pretty_string()).split(' ')[1:])
                            print(f"{x.host_mem_conf.host_memory_capacity.to_bytes()}")
                    else:
                        print(f"Accelerator does not match: {x.accelerator_conf.pretty_string()} vs {elem.value.accelerator_conf.pretty_string()}")
                else:
                    print(f"CPU does not match: {x.host_cpu_conf.pretty_string()} vs {elem.value.host_cpu_conf.pretty_string()}")
        return None
