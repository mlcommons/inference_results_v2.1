...
if [ "$#" -lt 4 ]; then
  echo "Use command: rn50_compliance.sh [perf|accu|sub] [Offline|Server] [systemid] [TEST01|TEST04|TEST05]"
  exit 1
fi
...
logdir="/work/compliance/"${systemid}"/resnet50/"${scenario}"/"${testid}"/"${testLog}
mkdir -p ${logdir}
