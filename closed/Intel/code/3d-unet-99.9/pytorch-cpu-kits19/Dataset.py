...
class Dataset(object):
    ...
    def get_warmup_samples(self):
        ...
        for _ in range(num_samples):
            data = self.get_qsl().get_features(random.randint(0,self.count-1))
            data = np.array([data])
    ...
    def get_samples(self, sample_index_list):
        # print("sample_index_list: ", sample_index_list)
        batch_size = len(sample_index_list)
        data = []
        for i in range(batch_size):
            data.append(self.get_qsl().get_features(sample_index_list[i]))
            if self.verbose:
                print("Processing sample id {:d} with shape = {:}, sub_cnt = {:d}".format(sample_index_list[i], data[i].shape, self.sample_id_subcnt[sample_index_list[i]]))
