...
def main():
    ...
    bs_args2formats = gen_by_batches_rn50(ctx, args, ["9", "4", "2"])
...
def get_rn50_backbone(ctx, batch_size: str):
    ...
    if batch_size == '112':
        get_cfgs = graph_configs.get_rn50_single_instance_bs112_cfgs
    elif batch_size == '9':
        get_cfgs = graph_configs.get_rn50_multi_instance_bs9_cfgs
    elif batch_size == '4' or batch_size == '2':
        get_cfgs = graph_configs.get_rn50_multi_instance_bs9_cfgs
        print("use bs=9's config for bs=2 and 4 for early evaluation")
    else:
...
def gen_rn50_backbone(ctx, args, batch_size: str, reorders: dict):
    ...
    # with open("src_"+batch_size+".cpp", "w") as f:
    #     f.write(main_src)
