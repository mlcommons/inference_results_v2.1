...
void SUT::instanceThread(){
    ...
    at::Tensor input_tensor = (torch::ones({this->batch_size_, Start_In_C, Start_I_H, Start_I_W})*10).to(torch::kInt8).to(torch::MemoryFormat::ChannelsLast);
    at::Tensor backbone_output = torch::zeros({ this->batch_size_, 2048, 7, 7 }).to(torch::kInt8).to(torch::MemoryFormat::ChannelsLast);
    int8_t* input_pointer = net->getInputPtr();
    {
        ...
        if (this->num_warmup_==0){
            ...
            for (int i=0; i < this->num_warmup_; i++){
                SUT::ds_->GetSamplesPointer(batch_idxs, input_pointer);
                // SUT::ds_->GetSamplesTensor(batch_idxs, input_tensor);
               
                torch::jit::IValue out = net->predict(input_tensor, backbone_output);
            }
    ...
    while (true){
        ...
        SUT::ds_->GetSamplesPointer(qitem.sample_idxs_, input_pointer);
        // SUT::ds_->GetSamplesTensor(qitem.sample_idxs_, input_tensor);

        // Inference step
        torch::jit::IValue outputs = net->predict(input_tensor, backbone_output);
...
void SUTServer::instanceThread(){
    ...
    // Warmup section
    at::Tensor input_tensor = torch::zeros({this->batch_size_, Start_In_C, Start_I_H, Start_I_W}).to(torch::kInt8).to(torch::MemoryFormat::ChannelsLast);
    at::Tensor backbone_output = torch::zeros({ this->batch_size_, 2048, 7, 7 }).to(torch::kInt8).to(torch::MemoryFormat::ChannelsLast);
    int8_t* input_pointer = net->getInputPtr();
    {
        std::unique_lock<std::mutex> lock(this->warmup_mutex_);
        ...
        if (this->num_warmup_==0){
            ...
            for (int i=0; i < this->num_warmup_; i++){
                ...
                SUTServer::ds_->GetSamplesPointer(batch_idxs, input_pointer);
    ...
    while (true){
        ...
        SUTServer::ds_->GetSamplesPointer(qitem.sample_idxs_, input_pointer);
