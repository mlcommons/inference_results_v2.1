...
const at::Tensor PreProcessSample(std::string filename, int height, int width){
    ...
    tensor = tensor.to(at::kFloat).contiguous();
...
void Dataset::GetSamplesTensor(std::vector<mlperf::QuerySampleIndex>& samples, at::Tensor& tensor){
    float* tensor_pointer = tensor.data_ptr<float>();
    #pragma omp parallel// shared(num_samples, tensor_pointer)
    {
        for (size_t i = 0; i < samples.size(); i++) {
            float* data_ptr = Dataset::data_in_memory_[samples[i]].data_ptr<float>();
            memcpy(tensor_pointer+i*3*800*800, data_ptr, 3*800*800*sizeof(float));
        }
    }
}
...
RetinaNetDataset::RetinaNetDataset(const std::string data_path, size_t total_sample_count) : Dataset(data_path, total_sample_count){
    ...
    //std::vector<at::Tensor> dboxes_xywh = dboxes.GetXYWH(); // [cx, cy, w, h] (num_feats x 4)
...
void RetinaNetDataset::PostProcess( Item qitem, torch::jit::IValue& output){
    ...
    auto logits = output.toTuple()->elements()[0].toTuple()->elements();
    ...
    auto boxes = output.toTuple()->elements()[1].toTuple()->elements();
...
std::vector<at::Tensor> RetinaNetDataset::DecodeSingle(std::vector<at::Tensor>& bboxes, std::vector<at::Tensor>& logits){
    ...
    long int num_topk;
    for (size_t i=0; i < bboxes.size(); i++){ // i < per_level_boxes; i++){
        level_anchors = this->dboxes_xywh_[i]; //split_anchors_[0][i]; // Get anchors at this feature level
        level_boxes = bboxes[i]; // BBoxes at this feature level
        scores_per_level = logits[i]; // For Logits at this feature level

        keep_idxs = scores_per_level.greater(this->score_thresh_); // Threshold the logits

        topk_idxs = at::where(keep_idxs)[0];
        scores_per_level = scores_per_level.index_select(0,topk_idxs);

        num_topk = std::min(topk_idxs.size(0),this->topk_candidates_); // limit no. of topk per lev

        auto topk_out = scores_per_level.topk(num_topk); // Compute topk

        scores_per_level = std::get<0>(topk_out); // Topk scores
        idxs = std::get<1>(topk_out);
        if (idxs.size(0) < 1) continue;

        topk_idxs = topk_idxs.index_select(0,idxs); // Select the topk indexes

        auto anchor_idxs = at::div(topk_idxs, this->num_classes_,"floor");// Map selected topk indicesto their corresponding anchors boxes. An anchor box may be repeated (typical) if multiple class scoresfor a particular box passes the 'score_thres_'
        labels_per_level = topk_idxs.remainder(this->num_classes_); // Map selected indices to their correct classes

        topk_boxes = level_boxes.index_select(0,anchor_idxs);

        /* Scale the predicted boxes at this feature level */
        scaled_bboxes = RetinaNetDataset::ScaleFeatureBboxes(topk_boxes, anchor_idxs, i);
        image_boxes.push_back(scaled_bboxes);
        image_scores.push_back(scores_per_level);
