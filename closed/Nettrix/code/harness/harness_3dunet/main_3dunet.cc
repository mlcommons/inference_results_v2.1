...
void doInference()
{
    ...
    // FIXME: GPUBatchSize/DLABatchSize fixed to 1
    // FIXME: current version only supports batch size == 1, copy_streams == 1
    CHECK_EQ(sut_settings.GPUBatchSize, 1) << "Batch size == 1 is supported only for now";
    ...
    CHECK_EQ(sut_settings.DLABatchSize, 1) << "Batch size == 1 is supported only for now";
