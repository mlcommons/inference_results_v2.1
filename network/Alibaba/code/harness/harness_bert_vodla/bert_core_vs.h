...
{
public:
    BERTCoreVS(const BERTCoreVS& other, int profileIdx);
    BERTCoreVS(const std::string& enginePath, int numCopyStreams, int numDupProfiles, int profileIdx, bool useGraphs,
        int deviceId, int maxBS);
    ~BERTCoreVS();
...
private:
    odla_context GetContext(int batchSize, int seqLen);
    void SetInputShapes(odla_context context, int sumS, int B, int maxS);
    std::map<std::pair<int, int>, void*> mMaxDimsToContext;
    size_t mMaxBatchSize;
    int mDeviceId;
    odla_executable mExec;
    odla_context mCtx;
    odla_resource_location mResLoc;
    std::shared_ptr<_odla_device> mDevice{nullptr, [](odla_device dev) {}};
    ...
    // we create a context per profile but share the device memory between them, assuming that we
    // use only one context at a time
    ...
    odla_value mInputIds;
    odla_value mSegmentIds;
    odla_value mCuSeqLens;
    odla_value mMaxSeqLens;
    odla_value mOutput;
