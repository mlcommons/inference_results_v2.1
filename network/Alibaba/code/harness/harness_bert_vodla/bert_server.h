...
class BERTManagedBuffer
{
  public:
    BERTManagedBuffer(size_t sz)
        : mSize(sz), mBytes(sizeof(T) * sz), mHostBuffer(mBytes),
...
class TotalLengthMultiSet
{
  public:
    ...
    void InsertTasks(const BERTTask_t& tasks, std::shared_ptr<qsl::SampleLibrary> qsl)
    {
        ...
        {
            BERTInput* mask =
                static_cast<BERTInput*>(qsl->GetSampleAddress(tasks[i].first.index, 2));
...
class BERTServer : public mlperf::SystemUnderTest
{
  public:
    BERTServer(const std::string name, const std::string enginePath,
               ...
               int graphsMaxSeqLen, const std::string& graphSpecs, double softDrop,
               double targetLatencyPercentile, uint64_t serverNumIssueQueryThreads);
  ...
  private:
    ...
    std::vector<
        std::deque<std::pair<mlperf::QuerySample, std::chrono::high_resolution_clock::time_point>>>
        mTasksVec;
