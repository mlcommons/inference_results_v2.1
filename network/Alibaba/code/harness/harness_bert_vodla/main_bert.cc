...
DEFINE_uint32(devices, 1, "Number of GPU devices to use");
...
DEFINE_string(graph_specs, "",
    "Specify a comma separeated list of (maxSeqLen, min totSeqLen, max totSeqLen, step "
    "size) for CUDA graphs to be captured");
...
int main(int argc, char* argv[])
{
    ...
    const std::string gSampleName = "BERT_HARNESS (VODLA)";
    ...
    // Scope to force all smart objects destruction before CUDA context resets
    {
        ...
        LOG(INFO) << "loadgen Version:" << mlperf::LoadgenVersion();
        LOG(INFO) << "loadgen Git Revision:" << mlperf::LoadgenGitRevision();

        std::vector<int> gpus;
        LOG(INFO) << "Use number of GPUs: " << FLAGS_devices;
        gpus.resize(FLAGS_devices);
        ...
        auto bert_server = std::make_shared<BERTServer>("BERT SERVER (Network SUT)", FLAGS_gpu_engines, qsl, gpus,
