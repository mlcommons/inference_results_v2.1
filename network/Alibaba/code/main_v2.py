...
This is a beta version of the new program entrypoint, which is built on top of the new API components that were added in
v1.1, v2.0, and v2.1.
"""
...
if __name__ == "__main__":
    ...
    system_id = main_args["system_name"]
    if system_id is None:
        logging.info(f"Please use --system_name to allocate a system. Exiting. {DETECTED_SYSTEM}")
    else:
        logging.info(f"Detected system ID: {system_id}")
        # main_args = parse_main_args()
        # system_id = main_args["system_name"]
        from code.common.systems.custom_list import custom_systems
        DETECTED_SYSTEM=custom_systems[str(system_id)]
