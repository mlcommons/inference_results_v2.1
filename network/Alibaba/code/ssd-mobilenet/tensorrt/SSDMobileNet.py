...
def mergeLocConfConv(network, index):
    loc_conv_name = f"BoxPredictor_{index}/BoxEncodingPredictor/Conv2D"
    conf_conv_name = f"BoxPredictor_{index}/ClassPredictor/Conv2D"
    loc_bias_name = f"BoxPredictor_{index}/BoxEncodingPredictor/biases/read"
    conf_bias_name = f"BoxPredictor_{index}/ClassPredictor/biases/read"
    ...
    merged_conv.name = f"BoxPredictor_loc_conf_{index}"
...
def preprocess(dynamic_graph):
    ...
    for i in range(0, 6):
        dynamic_graph.remove(dynamic_graph.find_nodes_by_path(f"BoxPredictor_{i}/stack"))
        dynamic_graph.forward_inputs(dynamic_graph.find_nodes_by_path(f"BoxPredictor_{i}/Reshape"))
        dynamic_graph.remove(dynamic_graph.find_nodes_by_path(f"BoxPredictor_{i}/stack_1"))
        dynamic_graph.forward_inputs(dynamic_graph.find_nodes_by_path(f"BoxPredictor_{i}/Reshape_1"))
        dynamic_graph.remove(dynamic_graph.find_nodes_by_path(f"BoxPredictor_{i}/Shape"))
...
class SSDMobileNet(TensorRTEngineBuilder):

    def __init__(self, args):
        workspace_size = dict_get(args, "workspace_size", default=(2 << 31))
        logging.info(f"Using workspace size: {workspace_size}")

        super().__init__(args, Benchmark.SSDMobileNet, workspace_size=workspace_size)
    ...
    def postprocess(self, replace_relu6=False):
        ...
        for i in range(nb_layers):
            ...
            logging.debug(f"({i}) Layer '{layer.name}' -> Type: {layer.type} ON {self.builder_config.get_device_type(layer)}")

            if replace_relu6 and "Relu6" in layer.name:
                activation = layer
                activation.__class__ = trt.IActivationLayer
                logging.debug(f"\tType: {activation.type}, alpha={activation.alpha}, beta={activation.beta}")
                # Convert to RELU
