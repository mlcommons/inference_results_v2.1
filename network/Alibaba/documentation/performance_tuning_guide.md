...
- start_from_device (in bytes), where tput is your throughput as 'Queries per second':
    ...
    - 3D-UNET: throughput x (inputC x D x H x W) = throughput x (1 x 264 x 346 x 349) = tput x 31879056
        - Note that, 3D-UNET KiTS19 input set has 42 inputs with 15 different shapes. The above D,H,W is the average shape from the 42 inputs (as opposed to the use of max D,H,W for the worst case scenario). This is reasonable because benchmark requires to run whole inputs multiple times, i.e. multiple iterations of the entire 42 samples sorted in random order per iteration, to avoid skewed results from non-uniform selection of inputs.
...
- end_on_device (in bytes), where tput is your throughput as 'Queries per second':
    - 3D-UNET: throughput x (outputC x D x H x W x sizeof(float16)) = throughput x (3 x 264 x 346 x 349 x 2) = tput x 191274336
