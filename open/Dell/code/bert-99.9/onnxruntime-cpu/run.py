...
def get_args():
    ...
    parser.add_argument(
        "--backend",
        choices=["tf", "pytorch", "onnxruntime", "tf_estimator", "openvino"],
        default="openvino",
        help="Backend",
    )
    parser.add_argument(
    ...
    parser.add_argument("--inter_threads", type=int, default=4, help="inter")
    parser.add_argument("--intra_threads", type=int, default=32, help="intra")
    parser.add_argument("--e_mode", type=str, default="para", help="execution")
    parser.add_argument(
        "--worker_threads",
        type=int,
        default=128,
        help="The number of threads to spwan with a single inference engine. Each thread reads a frame, runs inference, and uploads the results respectively.",
    )
    parser.add_argument(
        "--queue_max_size",
        type=int,
        default=512,
        help="The maximum number of requests that can be put in a queue before blocking the main thread.",
    )
    parser.add_argument("--model_path", type=str, default=None, help="path to model")
    parser.add_argument(
        "--log_path", type=str, default="build/logs", help="path to write_logs"
    )
    parser.add_argument(
        "--tokenizer",
        type=str,
...
def main():
    ...
    if args.accuracy:
        ...
        cmd = (
            ...
            + " --out_file "
            + os.path.join(log_path, "predictions.json")
            + " --log_file "
            + os.path.join(log_path, "mlperf_log_accuracy.json")
        )
        print(f"Run: {cmd}")
