...
def write_predictions(all_examples, all_features, all_results, n_best_size,
    ...
    for result in all_results:
        if result.unique_id not in unique_id_to_result:
    ...
    for (example_index, example) in enumerate(all_examples):
        ...
        for (feature_index, feature) in enumerate(features):
            ...
            results = unique_id_to_result.get(feature.unique_id, None)
            if results is None:
                continue
    ...
    for prediction in predictions:
        ...
        if output_transposed:
            logits = np.frombuffer(bytes.fromhex(
                prediction["data"]), dtype).reshape(-1, 2, 384)
            logits = np.transpose(logits)
        else:
            logits = np.frombuffer(bytes.fromhex(
                prediction["data"]), dtype).reshape(-1, 384, 2)
        # Pad logits to max_seq_length
        for j in range(logits.shape[0]):
            logit = logits[j, :, :]
            seq_length = logit.shape[0]
            start_logits = np.ones(max_seq_length) * -10000.0
            end_logits = np.ones(max_seq_length) * -10000.0
            start_logits[:seq_length] = logit[:, 0]
            end_logits[:seq_length] = logit[:, 1]
